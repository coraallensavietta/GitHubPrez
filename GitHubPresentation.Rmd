...
- Whenever you'd like to add to your history, you take a snapshot, called a "commit", and add a message.

- To look at a previous version of a project, simply check out the previous commit, then go come back to the newest version.
...
**Step 7**: Add your first commit
We'll each add a new file to the repository, which we'll name with our initials: e.g. yourinitials.txt.    
**Please don't use a generic file name -- this could create a giant 20-way conflict.**  
```{bash, echo = TRUE, eval=FALSE}
cd testcommits # go into the testcommmits directory 
touch your-file-name # use this file name: yourinitials.txt 
git add your-file-name # stage your new file using its file name (e.g. yourinitials.txt)
git status
